...
- Release status: commit `2353fbd` was deployed to the existing `liewcf-org` Cloudflare Pages project on 2026-07-26 by direct upload of the verified `dist/` output. Production checks confirmed 200 responses for Home, About, Projects, Updates, RSS, generated Markdown, and the API catalog; 404 responses for a draft Project and an unknown route; and the permanent `www` redirect. The Git-triggered deployment for the same commit served 404, so its Cloudflare build configuration still needs separate investigation before relying on automatic deployments.
