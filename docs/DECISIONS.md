...
- Use required typed Project fields for title, summary, GitHub repository URL, categories, featured state, and draft state. Limit optional metadata to status, a credential-free HTTP(S) live URL, and a stable `/uploads/` cover-image URL, rendering each only when present.
