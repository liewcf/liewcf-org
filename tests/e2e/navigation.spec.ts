...
test('shared navigation exposes only live destinations and identifies the current page', async ({ page }) => {
	for (const [path, currentPage] of [
		...
		await expect(navigation.getByRole('link', { name: currentPage === 'Home' ? 'About' : 'Home' })).not.toHaveAttribute('aria-current');
		await expect(navigation.getByRole('link', { name: /Projects|Updates/ })).toHaveCount(0);
	}
});
...
test('homepage and about remain readable without horizontal overflow', async ({ page }) => {
...
test('removed routes do not behave like live site pages', async ({ page }) => {
	for (const path of ['/blog/', '/projects/', '/contact/']) {
