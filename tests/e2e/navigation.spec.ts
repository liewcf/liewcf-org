...
test('established public asset, crawler, well-known, and agent URLs resolve from production output', async ({ request }) => {
	for (const path of [
		'/favicon.ico',
		'/assets/favicon.svg',
		'/assets/apple-touch-icon.png',
		...
		expect(response.status(), `${path} should resolve from the production preview`).toBe(200);
	}
});
...
test('representative page types remain keyboard-usable and readable without horizontal overflow', async ({ page }) => {
	const paths = [
		['/', true],
		['/about/', true],
		['/projects/', true],
		['/projects/youtube-watchlist-manager/', true],
	...
	for (const viewport of [
		...
		for (const [path, hasPrimaryNavigation] of paths) {
			await page.goto(path);
			await expect(page.getByRole('navigation', { name: 'Primary navigation' })).toHaveCount(hasPrimaryNavigation ? 1 : 0);
			await expect(page.getByRole('heading', { level: 1 })).toBeVisible();
			if (hasPrimaryNavigation) {
				const hiddenSkipLink = page.getByRole('link', { name: 'Skip to content' });
				const box = await hiddenSkipLink.boundingBox();
				expect(box && box.y + box.height).toBeLessThanOrEqual(0);
			}
			await page.keyboard.press('Tab');
			const firstKeyboardLink = hasPrimaryNavigation
				? page.getByRole('link', { name: 'Skip to content' })
				: page.getByRole('link', { name: 'Go home' });
			await expect(firstKeyboardLink).toBeFocused();
			await expect(firstKeyboardLink).toHaveCSS('outline-style', 'solid');
...
test('Cloudflare keeps contact redirect while former Blog routes remain real 404s', async () => {
	...
	expect(redirects).toBe(sourceRedirects);
	expect(rules).toEqual([
		'/.well-known/api-catalog /.well-known/api-catalog.json 200',
		'/contact/ / 301',
	]);
	for (const liveOrRemovedRoute of ['/about/', '/projects/', '/updates/', '/blog/']) {
		expect(redirects).not.toContain(`${liveOrRemovedRoute} `);
	}
});
...
test('cloudflare headers include security policy protections', async () => {
	...
	expect(headers).toBe(sourceHeaders);
	for (const protection of [
		'Strict-Transport-Security: max-age=31536000; includeSubDomains',
		"Content-Security-Policy: default-src 'self'; base-uri 'self'; object-src 'none'; script-src 'self' 'unsafe-inline'; style-src 'self'; img-src 'self'; font-src 'self'; connect-src 'self'; form-action 'none'; frame-ancestors 'none'; upgrade-insecure-requests",
		...
		expect(headers).toContain(protection);
	}
	...
	for (const contentType of [
		...
		expect(headers).toContain(contentType);
	}
});
...
test('production stays static and excludes Blog, CMS, admin, authentication, and runtime configuration', async () => {
	...
	const sourcePaths = runtimePaths.filter((path) => /\.(astro|json|md|mjs|ts|txt)$/.test(path));
	const sourceFiles = (await Promise.all(sourcePaths.map((path) => readFile(path, 'utf8')))).join('\n').toLowerCase();
	const directDependencies = {
		...JSON.parse(packageJson).dependencies,
		...JSON.parse(packageJson).devDependencies,
	};

	for (const excludedPath of ['admin', 'functions', 'api/contact', 'netlify', 'decap']) {
		expect(
			runtimePaths.some((path) => new RegExp(`(^|/)${excludedPath}(/|$)`).test(path.toLowerCase())),
			`${excludedPath} should not exist in the production source tree`,
		).toBe(false);
	}
	for (const excluded of ['decap', '/admin/', 'authentication', 'comments', 'reactions']) {
		expect(sourceFiles).not.toContain(excluded);
		expect(packageJson.toLowerCase()).not.toContain(excluded);
	}
	for (const excludedDependency of ['@astrojs/cloudflare', '@astrojs/node', 'decap-cms-app', 'netlify-cms-app']) {
		expect(directDependencies).not.toHaveProperty(excludedDependency);
	}
	expect(astroConfig).toContain("output: 'static'");
	expect(astroConfig).not.toContain('adapter:');
	expect(sourceFiles).not.toContain('process.env');
	expect(sourceFiles).not.toContain('blog');
	await expect(readFile('dist/admin/index.html', 'utf8')).rejects.toThrow();
});
